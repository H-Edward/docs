...
Under Debian 13 `Trixie` and Ubuntu 25 `Plucky Puffin` official packages are maintained and therefore can be installed with the following commands:
...
However for those using older versions of Debian, Ubuntu or Raspberry Pi OS, instructions for installation can be found on the official wiki for `dnscrypt-proxy` [here](https://github.com/DNSCrypt/dnscrypt-proxy/wiki/Installation-linux#installation-on-linux-overview).
